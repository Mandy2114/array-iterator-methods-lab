...
// Complete the exercise in the space below:
const isAdultPresent =  devs.some(dev => dev.year <= 2006 )
...
// Complete the exercise in the space below:
const isEveryone19OrOlder = devs.every(person => (2024 - person.year ) >= 19);
...
// Complete the exercise in the space below:
const totalYearsLived = inventors.reduce((accumulate, inventor) => 
  { const lifespan = inventor.passed - inventor.year;
return accumulate + lifespan;
  }, 0
);
...
// // Complete the exercise in the space below:
const travelMethodCounts = travelMethods.reduce((accumulator, method) => {
